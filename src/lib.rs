...
lazy_static! {
    ...
    static ref OBSERVER_COLLECTION: Mutex<ObserverCollection> =
        Mutex::new(ObserverCollection::new());
}
...
impl ObserverCollection {
    ...
    pub fn add_observer<F>(&mut self, notification: String, f: F) -> Token
    where
    ...
    pub fn observers_for_notification(
        &self,
        notification: String,
...
impl NotificationCenter {
    ...
    pub fn observe<F>(notification: String, f: F) -> Token
    where
        F: Fn() + Send + Sync + 'static,
    {
        OBSERVER_COLLECTION
            ...
            .unwrap()
            .borrow_mut()
            .add_observer(notification, f)
    }
    pub fn post(notification: String) {
        if let a = OBSERVER_COLLECTION
            .lock()
            .unwrap()
            .observers_for_notification(notification)
        {
            for c in a {
